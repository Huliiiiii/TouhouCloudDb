...
router.get("/song:id", async function (req, res) {
	try {
		...
		const [albumResult] = await pool.query("SELECT * FROM touhoudbtest.albums WHERE JSON_CONTAINS(Tracklist, CAST(? AS JSON), '$');", [
		...
		res.render("song_page", {songResult: songResult[0], albumResult, artistResult});
	} catch (error) {
		console.error(error);
