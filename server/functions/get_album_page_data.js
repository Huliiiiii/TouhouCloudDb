...
async function get_album_page_data(album_id) {
	...
	try {
		const [album_result] = await pool.query(
			`SELECT * FROM touhoudbtest.albums WHERE albumID = ? AND isDelete = 0`,
			album_id_str
		);
		...
		if (album_data.artist) {
			...
			try {
				[artist_result] = await pool.query(
					"SELECT * FROM touhoudbtest.artists WHERE artistID IN (?) AND isDelete = 0",
					[album_artist_id]
				);
			} catch (error) {
				console.error("Error in getting artist data:", error);
			}
		}
		...
		const [track_listing, _] = await pool.query(
			`SELECT * FROM touhoudbtest.songs WHERE songID IN (${track_listing_id}) AND isDelete = 0`
		);
